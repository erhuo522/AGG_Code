...
#error "No appropriate constant for alpha format. Check version of wingdi.h, There must be AC_SRC_ALPHA or AC_SRC_NO_PREMULT_ALPHA"
	...
	void    pixel_map::copy_bits(unsigned char* data, bool flp)
	{
	 
		if(flp)
		{
			memcpy(m_buf,data,m_img_size);
		}
		else
		{
			int nSwap = m_bmp->bmiHeader.biHeight-1;
			int row_len = stride();

			for(int i=nSwap; i>0; i--)
			{
				memcpy(m_buf + i*row_len , data, row_len);
				data += row_len ; 
			}	
		}
	}
